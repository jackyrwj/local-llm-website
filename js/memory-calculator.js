...
function populateSelects() {
  ...
  MODEL_PRESETS.forEach(m => {
    ...
    opt.value = m.id;
    opt.textContent = `${m.name} (${m.params}B, ${m.hiddenSize}dim, ${m.layers}层)`;
...
function getGpu() {
  ...
  return GPU_PRESETS.find(g => g.vram === vram) || { name: `自定义 ${vram}GB`, vram };
}
...
function renderResult(r) {
  // 状态卡片
  const statusLabels = { safe: '✅ 显存充足', warning: '⚠️ 余量紧张', danger: '❌ 显存不足' };
  ...
  setSeg(els.segWeight, wPct, r.weightGb, '权重');
  setSeg(els.segKv, kPct, r.kvCacheGb, 'KV');
  setSeg(els.segActivation, aPct, r.activationGb, '激活');
  setSeg(els.segOverhead, oPct, r.overheadGb, '开销');
...
function renderSuggestions(r) {
  ...
  if (r.status === 'safe') {
    if (r.marginRatio > 0.3) {
      list.push('显存余量充足，可以考虑增大上下文长度或并发数来提升吞吐。');
    }
    if (r.kvCacheGb > r.weightGb * 1.5) {
      list.push('KV Cache 占比较大，如果不需要长上下文，可以适当降低以节省显存。');
    }
  }
  ...
  if (r.status === 'danger' || r.status === 'warning') {
    ...
    if (availableForKv > 0) {
      ...
      if (maxCtx >= 512 && maxCtx < r.seqLen) {
        list.push(`将上下文长度降到 ${maxCtx.toLocaleString()} 可以在当前配置下装下。`);
      }
    }
    ...
    for (const q of quants) {
      ...
      if (newTotal < r.availableGb) {
        ...
        list.push(`改用 ${q.label} 量化可节省 ${saved.toFixed(1)} GB 显存。`);
        break; // 只建议第一个可行的
      }
    }
    ...
    if (availableForKv2 > 0) {
      ...
      if (maxConcurrency >= 1 && maxConcurrency < r.batchSize) {
        list.push(`将并发请求数降到 ${maxConcurrency} 可以在当前配置下装下。`);
      }
    }
    ...
    if (betterGpu && betterGpu.vram > r.gpu.vram) {
      list.push(`需要至少 ${betterGpu.vram}GB 显存的 GPU（如 ${betterGpu.name}）。`);
    } else if (!betterGpu) {
      list.push(`当前单卡配置无法满足，建议增加 GPU 数量或使用多卡并行。`);
    }
  }
